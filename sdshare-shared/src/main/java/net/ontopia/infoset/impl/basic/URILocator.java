  ...
  */
  ...
  public URILocator(File file) {
    try {
      ...
      throw new SDShareRuntimeException("INTERNAL ERROR: File " + file +
  ...
  public LocatorIF resolveAbsolute(String rel) {
    ...
    case '/':
      if (length != 1 && rel.charAt(1) == '/') { // begins with "//"
        if (authorityEnd == -1)
          throw new SDShareRuntimeException(new MalformedURLException("Base URI is not hierarchical"));
    ...
    for (int ix = 0; ix < path.length(); ix++) {
      ...
      if ((ch >= 'a' && ch <= 'z') ||
        ...
        if (ch < 0x07FF) {
          ...
          throw new SDShareRuntimeException("INTERNAL ERROR: Only BMP characters supported");
      } else {
        tmp[pos++] = '%';
